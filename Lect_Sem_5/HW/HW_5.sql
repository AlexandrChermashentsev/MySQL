DROP DATABASE IF EXISTS sem_5_hw;
CREATE DATABASE sem_5_hw;
USE sem_5_hw;
...
-- CREATE VIEW example_1 AS
-- WITH users_name AS (SELECT)
CREATE OR REPLACE VIEW example_1 AS
SELECT 
...
WHERE YEAR(CURRENT_DATE) - YEAR(profiles.birthday) < 20;

/* 2. Найдите кол во отправленных сообщений каждым пользователем и выведите ранжированный список пользователей, указав имя и фамилию пользователя, количество
...
SELECT 
	DENSE_RANK() OVER (ORDER BY COUNT(messages.from_user_id) DESC) AS 'Rang',
    CONCAT(users.firstname, ' ', users.lastname) AS 'full name',
	COUNT(messages.from_user_id) AS 'Count messages'
FROM users
...
найдите разницу времени отправления между соседними сообщениями, получившегося списка. (используйте LEAD или LAG)
*/
SELECT 	
	body,
    created_at,
