...
UPDATE `profiles` SET hometown = 'Howeside' WHERE user_id = 1;
-- ----------------------- Задачи и решение -------------------------------------------------------------------------------------------------------------------
...
CALL moving(10); -- Вызов процедуры moving
...
DELIMITER //
CREATE FUNCTION hello_function() 
RETURNS VARCHAR(20)
DETERMINISTIC
BEGIN
	...
	SELECT CASE 
		...
		WHEN CURRENT_TIME >= '06:00:00' AND  CURRENT_TIME < '12:00:00' 
			THEN 'Доброе утро'
		WHEN CURRENT_TIME >= '00:00:00' AND  CURRENT_TIME < '06:00:00' 
			THEN 'Доброй ночи'
		ELSE 'Добрый вечер'
	END INTO hello_text;
	RETURN hello_text;
...
SELECT CURRENT_TIME() AS 'Текущее время', hello_function() AS 'Приветствие'; -- Вызов функции hello_function